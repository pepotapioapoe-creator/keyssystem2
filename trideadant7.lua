...
local settings = {
    ...
    espBox = true, espTracer = true, skeleton = false, espNpcs = true,
    tracerOrigin = "Bottom",
...
titleLbl.Text = "Tridentes" titleLbl.Parent = topBar
...
createToggle(c2, "Esqueleto", function(v) settings.skeleton = v end)
...
TCONN(RunService.RenderStepped:Connect(function(dt)
    ...
    local okLoop, errLoop = pcall(function()
        ...
        -- ESP: cuerpos custom de Const/Ignore
        espFrame = espFrame + 1
        if settings.espEnabled then
            local targets = {}
            local folder = getBodyFolder()
            if folder then
        ...
        do
            ...
            elseif not aiming then
                ...
                if myP then
                    ...
                    if folder then
                        for _, m in ipairs(folder:GetChildren()) do
                            if isBody(m) then consider(m, "Jugador") end
                        end
                    end
                end
                if bestPart then
