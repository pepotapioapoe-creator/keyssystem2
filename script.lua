...
--[[
    ZVOLT HUB - FULL VERSION (FIXED ESP BOX & CTRL+CLICK TOGGLE)
]]--
...
local settings = {
	...
	hitboxEnabled = false,
	spinEnabled = false,
	ctrlClickTpEnabled = false, -- Opción Misc para activar/desactivar Ctrl + Click TP
	targetPart = "Head",
	hitboxSize = 5,
	spinSpeed = 50,
	smoothing = 0.3,
...
local tabNames = {"combat", "visuals", "weapon", "teleport", "misc", "keybinds"}
...
-- PESTAÑA: COMBAT
local cardCombatGen = createCard(pages["combat"], "General", 10, 10, 680, 145)
...
createSlider(cardCombatGen, 72, settings.hitboxSize, 2, 20, "Hitbox Size", function(val) settings.hitboxSize = val end)

local cardCombatFov = createCard(pages["combat"], "FOV Settings", 10, 165, 680, 85)
...
local cardCombatSettings = createCard(pages["combat"], "Target Bone Selection", 10, 260, 680, 85)
...
-- PESTAÑA: MISC (Incluye el toggle para Ctrl + Click TP)
...
RunService.RenderStepped:Connect(function()
	...
	if settings.spinEnabled then
		...
		if rootPart then rootPart.CFrame = rootPart.CFrame * CFrame.Angles(0, math.rad(settings.spinSpeed), 0) end
	end
